...
Code.require_file("lib/emitters/rust.ex", ".")
...
{parsed, argv, _} = OptionParser.parse(System.argv(),
  switches: [language: :string, output: :string],
  aliases: [l: :language, o: :output]
)

language = Keyword.get(parsed, :language, "Rust")
output_dir = Keyword.get(parsed, :output, "rust/src/generated/")
# Ensure output_dir ends with /
output_dir = if String.ends_with?(output_dir, "/"), do: output_dir, else: output_dir <> "/"

IO.puts("Target Language: #{language}")
IO.puts("Output Directory: #{output_dir}")

Application.put_env(:asn1scg, :language, language)
...
ASN1.Emitters.Rust.finalize(output_dir)
