...
#[derive(Clone, Debug)]
pub enum LayoutDescriptorsMeasurePairVerticalChoice {
    Fixed(ASN1Integer),
    NotPresent(ASN1Null),
}

impl DERParseable for LayoutDescriptorsMeasurePairVerticalChoice {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (100, TagClass::ContextSpecific) => Ok(Self::NotPresent(ASN1Null::from_der_node(node)?)),
            (99, TagClass::ContextSpecific) => Ok(Self::Fixed(ASN1Integer::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
...
impl DERSerializable for LayoutDescriptorsMeasurePairVerticalChoice {
