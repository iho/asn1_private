...
pub enum KEPOtherHash {
    Sha1Hash(crate::KEPOtherHashValue),
    OtherHash(crate::KEPOtherHashAlgAndValue),
}

impl DERParseable for KEPOtherHash {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (37, TagClass::ContextSpecific) => Ok(Self::Sha1Hash(crate::KEPOtherHashValue::from_der_node(node)?)),
            (38, TagClass::ContextSpecific) => Ok(Self::OtherHash(crate::KEPOtherHashAlgAndValue::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
