...
impl DERParseable for AuthenticationFrameworkTime {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (63, TagClass::ContextSpecific) => Ok(Self::UtcTime(UTCTime::from_der_node(node)?)),
            (64, TagClass::ContextSpecific) => Ok(Self::GeneralizedTime(GeneralizedTime::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
