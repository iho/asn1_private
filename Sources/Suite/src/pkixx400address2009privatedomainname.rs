...
#[derive(Clone, Debug)]
pub enum PKIXX400Address2009PrivateDomainName {
    Numeric(ASN1NumericString),
    Printable(ASN1PrintableString),
}

impl DERParseable for PKIXX400Address2009PrivateDomainName {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (59, TagClass::ContextSpecific) => Ok(Self::Numeric(ASN1NumericString::from_der_node(node)?)),
            (60, TagClass::ContextSpecific) => Ok(Self::Printable(ASN1PrintableString::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
...
impl DERSerializable for PKIXX400Address2009PrivateDomainName {
