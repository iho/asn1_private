...
#[derive(Clone, Debug)]
pub enum DORDefinitionToken {
    Simpletoken(ASN1OctetString),
    Externaltoken(ASN1Node),
}

impl DERParseable for DORDefinitionToken {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (78, TagClass::ContextSpecific) => Ok(Self::Simpletoken(ASN1OctetString::from_der_node(node)?)),
            (80, TagClass::ContextSpecific) => Ok(Self::Externaltoken(node)),
            _ => Err(ASN1Error::new(
...
impl DERSerializable for DORDefinitionToken {
