...
#[derive(Clone, Debug)]
pub enum ColourAttributesRealOrInt {
    A(ASN1Real),
    B(ASN1Integer),
}

impl DERParseable for ColourAttributesRealOrInt {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (10, TagClass::ContextSpecific) => Ok(Self::A(ASN1Real::from_der_node(node)?)),
            (11, TagClass::ContextSpecific) => Ok(Self::B(ASN1Integer::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
...
impl DERSerializable for ColourAttributesRealOrInt {
