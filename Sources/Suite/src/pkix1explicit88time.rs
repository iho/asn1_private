...
#[derive(Clone, Debug)]
pub enum PKIX1Explicit88Time {
    UtcTime(UTCTime),
    GeneralTime(GeneralizedTime),
}

impl DERParseable for PKIX1Explicit88Time {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (265, TagClass::ContextSpecific) => Ok(Self::UtcTime(UTCTime::from_der_node(node)?)),
            (266, TagClass::ContextSpecific) => Ok(Self::GeneralTime(GeneralizedTime::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
...
impl DERSerializable for PKIX1Explicit88Time {
