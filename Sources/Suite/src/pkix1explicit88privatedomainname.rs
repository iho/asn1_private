...
#[derive(Clone, Debug)]
pub enum PKIX1Explicit88PrivateDomainName {
    Numeric(ASN1NumericString),
    Printable(ASN1PrintableString),
}

impl DERParseable for PKIX1Explicit88PrivateDomainName {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (360, TagClass::ContextSpecific) => Ok(Self::Numeric(ASN1NumericString::from_der_node(node)?)),
            (361, TagClass::ContextSpecific) => Ok(Self::Printable(ASN1PrintableString::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
...
impl DERSerializable for PKIX1Explicit88PrivateDomainName {
