...
pub enum DSTUBinaryFieldPChoice {
    T(crate::DSTUTrinomial),
    P(crate::DSTUPentanomial),
}

impl DERParseable for DSTUBinaryFieldPChoice {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (0, TagClass::ContextSpecific) => Ok(Self::T(crate::DSTUTrinomial::from_der_node(node)?)),
            (1, TagClass::ContextSpecific) => Ok(Self::P(crate::DSTUPentanomial::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
