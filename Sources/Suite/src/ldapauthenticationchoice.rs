...
pub enum LDAPAuthenticationChoice {
    ...
    Sasl(crate::LDAPSaslCredentials),
...
impl DERParseable for LDAPAuthenticationChoice {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (136, TagClass::ContextSpecific) => Ok(Self::Simple(ASN1OctetString::from_der_node(node)?)),
            (138, TagClass::ContextSpecific) => Ok(Self::Sasl(crate::LDAPSaslCredentials::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
