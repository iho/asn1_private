...
impl DERParseable for DSTUTime {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (60, TagClass::ContextSpecific) => Ok(Self::UtcTime(UTCTime::from_der_node(node)?)),
            (61, TagClass::ContextSpecific) => Ok(Self::GeneralTime(GeneralizedTime::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
