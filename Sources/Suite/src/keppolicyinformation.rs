...
pub struct KEPPolicyInformation {
    pub policy_identifier: crate::KEPCertPolicyId,
    pub policy_qualifiers: Option<Vec<KEPPolicyQualifierInfo>>,
}

impl KEPPolicyInformation {
    pub fn new(policy_identifier: crate::KEPCertPolicyId, policy_qualifiers: Option<Vec<KEPPolicyQualifierInfo>>) -> Self {
        Self {
            policy_identifier,
            policy_qualifiers,
        }
...
impl DERParseable for KEPPolicyInformation {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        der::sequence(node, ASN1Identifier::SEQUENCE, |nodes| {
            ...
            let nodes = &mut iter;
            let policy_identifier = crate::KEPCertPolicyId::from_der_node(nodes.next().ok_or(ASN1Error::new(rust_asn1::errors::ErrorCode::TruncatedASN1Field, "Premature end of data".to_string(), file!().to_string(), line!()))?)?;
            let policy_qualifiers: Option<Vec<KEPPolicyQualifierInfo>> = if let Some(node) = nodes.peek().map(|n| n.clone()) { if let rust_asn1::asn1::Content::Constructed(collection) = &node.content { nodes.next(); match collection.clone().into_iter().map(|child| KEPPolicyQualifierInfo::from_der_node(child)).collect::<Result<_, _>>() { Ok(v) => Some(v), Err(_) => None } } else { None } } else { None };
            Ok(Self {
