...
#[derive(Clone, Debug)]
pub enum TextUnitsTypeOfCoding {
    A(ASN1Integer),
    B(ASN1ObjectIdentifier),
}

impl DERParseable for TextUnitsTypeOfCoding {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (58, TagClass::ContextSpecific) => Ok(Self::A(ASN1Integer::from_der_node(node)?)),
            (59, TagClass::ContextSpecific) => Ok(Self::B(ASN1ObjectIdentifier::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
...
impl DERSerializable for TextUnitsTypeOfCoding {
