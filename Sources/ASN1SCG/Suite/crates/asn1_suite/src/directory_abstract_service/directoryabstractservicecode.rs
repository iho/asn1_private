...
impl DERParseable for DirectoryAbstractServiceCode {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (37, TagClass::ContextSpecific) => Ok(Self::Local(ASN1Integer::from_der_node(node)?)),
            (38, TagClass::ContextSpecific) => Ok(Self::Global(ASN1ObjectIdentifier::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
