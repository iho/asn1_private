...
#[derive(Clone, Debug)]
pub enum CryptographicMessageSyntax2009Time {
    UtcTime(UTCTime),
    GeneralTime(GeneralizedTime),
}

impl DERParseable for CryptographicMessageSyntax2009Time {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (407, TagClass::ContextSpecific) => Ok(Self::UtcTime(UTCTime::from_der_node(node)?)),
            (408, TagClass::ContextSpecific) => Ok(Self::GeneralTime(GeneralizedTime::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
...
impl DERSerializable for CryptographicMessageSyntax2009Time {
