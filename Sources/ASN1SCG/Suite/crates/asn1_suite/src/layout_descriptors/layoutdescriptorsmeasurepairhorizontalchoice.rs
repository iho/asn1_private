...
#[derive(Clone, Debug)]
pub enum LayoutDescriptorsMeasurePairHorizontalChoice {
    Fixed(ASN1Integer),
    NotPresent(ASN1Null),
}

impl DERParseable for LayoutDescriptorsMeasurePairHorizontalChoice {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (96, TagClass::ContextSpecific) => Ok(Self::Fixed(ASN1Integer::from_der_node(node)?)),
            (97, TagClass::ContextSpecific) => Ok(Self::NotPresent(ASN1Null::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
...
impl DERSerializable for LayoutDescriptorsMeasurePairHorizontalChoice {
