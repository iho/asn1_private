...
#[derive(Clone, Debug)]
pub enum CryptographicMessageSyntax2010Time {
    UtcTime(UTCTime),
    GeneralTime(GeneralizedTime),
}

impl DERParseable for CryptographicMessageSyntax2010Time {
    fn from_der_node(node: ASN1Node) -> Result<Self, ASN1Error> {
        match (node.identifier.tag_number, node.identifier.tag_class) {
            (424, TagClass::ContextSpecific) => Ok(Self::UtcTime(UTCTime::from_der_node(node)?)),
            (425, TagClass::ContextSpecific) => Ok(Self::GeneralTime(GeneralizedTime::from_der_node(node)?)),
            _ => Err(ASN1Error::new(
...
impl DERSerializable for CryptographicMessageSyntax2010Time {
